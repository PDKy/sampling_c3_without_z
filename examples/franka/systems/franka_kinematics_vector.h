...
//franka vector
template <typename T>
class FrankaKinematicsVector_franka : public TimestampedVector<T> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(FrankaKinematicsVector_franka)

  FrankaKinematicsVector_franka() = default;

  explicit FrankaKinematicsVector_franka(int num_end_effector_positions,
                                  int num_end_effector_velocities)
      : TimestampedVector<T>(num_end_effector_positions +
                             num_end_effector_velocities ),
        num_end_effector_positions_(num_end_effector_positions),
        num_end_effector_velocities_(num_end_effector_velocities),
        end_effector_positions_start_(0),
        end_effector_velocities_start_(num_end_effector_positions_ ),
        num_positions_(num_end_effector_positions_ ),
        num_velocities_(num_end_effector_velocities_ ) {
  }

  /// Constructs a OutputVector with the specified positions and velocities.
  explicit FrankaKinematicsVector_franka(
      const drake::VectorX<T>& end_effector_positions,

      const drake::VectorX<T>& end_effector_velocities)

      : FrankaKinematicsVector_franka(
            end_effector_positions.size(),
            end_effector_velocities.size()) {
    this->SetEndEffectorPositions(end_effector_positions);
