...
SamplingC3Controller::SamplingC3Controller(
    ...
    drake::multibody::MultibodyPlant<double>& plant_c3,
    drake::systems::Context<double>* context_c3,
    ...
    : plant_(plant),
      ...
      plant_c3_(plant_c3),
      context_c3_(context_c3),
      plant_ad_c3_(plant_ad_c3),
      context_ad_c3_(context_ad_c3),
      contact_pairs_c3_(contact_geoms_c3),
  ...
  //input to c3 without z
  n_q_c3_ = plant_c3_.num_positions();
  n_v_c3_ = plant_c3_.num_velocities();
  n_u_c3_ = plant_c3_.num_actuators();

  n_x_c3_ = n_q_c3_ + n_v_c3_;
  ...
  if (c3_options_.contact_model == "stewart_and_trinkle") {
    ...
    2 * c3_options_.num_friction_directions * sampling_c3_options_.num_contacts[sampling_c3_options_.num_contacts_index];
      ...
      n_lambda_ =
      2 * c3_options_.num_friction_directions * sampling_c3_options_.num_contacts[sampling_c3_options_.num_contacts_index]-2;
  ...
  auto x_desired_placeholder =
      std::vector<VectorXd>(N_ + 1, VectorXd::Zero(n_x_c3_));
  ...
  for (int i : vector<int>({0})) {
    Eigen::RowVectorXd A = VectorXd::Zero(n_x_c3_);
    A(i) = 1.0;
  ...
  for (int i : vector<int>({0, 1})) {
    Eigen::RowVectorXd A = VectorXd::Zero(n_u_c3_);
    A(i) = 1.0;
  ...
  c3_solution.x_sol_ = MatrixXf::Zero(n_q_c3_ + n_v_c3_, N_);
  c3_solution.lambda_sol_ = MatrixXf::Zero(n_lambda_, N_);
  c3_solution.u_sol_ = MatrixXf::Zero(n_u_c3_, N_);
  c3_solution.time_vector_ = VectorXf::Zero(N_);
  auto c3_intermediates = C3Output::C3Intermediates();
  c3_intermediates.z_ = MatrixXf::Zero(n_x_c3_ + n_lambda_ + n_u_c3_, N_);
  c3_intermediates.w_ = MatrixXf::Zero(n_x_c3_ + n_lambda_ + n_u_c3_, N_);
  c3_intermediates.delta_ = MatrixXf::Zero(n_x_c3_ + n_lambda_ + n_u_c3_, N_);
  c3_intermediates.time_vector_ = VectorXf::Zero(N_);
  auto lcs_contact_jacobian = std::pair(Eigen::MatrixXd(n_x_c3_, n_lambda_),
...
LCS SamplingC3Controller::CreatePlaceholderLCS() const {
  MatrixXd A = MatrixXd::Ones(n_x_c3_, n_x_c3_);
  MatrixXd B = MatrixXd::Zero(n_x_c3_, n_u_c3_);
  VectorXd d = VectorXd::Zero(n_x_c3_);
  MatrixXd D = MatrixXd::Ones(n_x_c3_, n_lambda_);
  MatrixXd E = MatrixXd::Zero(n_lambda_, n_x_c3_);
  MatrixXd F = MatrixXd::Zero(n_lambda_, n_lambda_);
  MatrixXd H = MatrixXd::Zero(n_lambda_, n_u_c3_);
...
drake::systems::EventStatus SamplingC3Controller::ComputePlan(
  ...
  //TODO change the x_desired to c3 len

  BasicVector<double> x_lcs_des_c3(n_x_c3_);

  if (sampling_c3_options_.with_z){
  x_lcs_des_c3.set_value(x_lcs_des.value());
  }else{
    Eigen::VectorXd x_lcs_des_vec(n_x_c3_);
    x_lcs_des_vec << x_lcs_des.value().head(2),x_lcs_des.value().segment(3,9),x_lcs_des.value().segment(13,6);
    x_lcs_des_c3.set_value(x_lcs_des_vec);
}


  std::vector<VectorXd> x_desired =
      std::vector<VectorXd>(N_ + 1, x_lcs_des_c3.value());
  ...
  if (sampling_c3_options_.use_quaternion_dependent_cost &&
    ...
    double discount_factor = 1;
    for (int i = 0; i < N_+1; ++i) {
      Q_[i].block(n_q_franka_,n_q_franka_,4,4) = discount_factor *
  ...
  std::vector<Eigen::VectorXd> candidate_states = generate_sample_states(
    n_q_, n_v_, n_u_, x_lcs_curr, is_doing_c3_, sampling_params_, c3_options_,sampling_c3_options_,
  ...
  //auto flag_coll = check_collision(x_lcs_curr,plant_,context_,contact_pairs_,sampling_params_,c3_options_);
  //check
  // Eigen::VectorXd x_curr_c3(n_x_);
  //
  // x_curr_c3.head(2) = x_lcs_curr.head(2);
  // x_curr_c3[2] = (float)sampling_c3_options_.c3_height;
  // x_curr_c3.segment(3,9) = x_lcs_curr.segment(2,9);
  // x_curr_c3[12] = (float)0;
  // x_curr_c3.segment(13,6) = x_lcs_curr.segment(11,6);
  // //
  // UpdateContext(x_curr_c3);
  ...
  std::cerr << "number_sample" << num_total_samples << std::endl;
  std::cerr << "number lambda" << n_lambda_ << std::endl;

// if (!is_doing_c3_) {
//   std::vector<Eigen::VectorXd> candidate_states_temp;
//   //Test used to check collsion
//   for (int i=0;i<num_total_samples;i++) {
//     Eigen::VectorXd candidate_states_c3(n_x_c3_);
//     candidate_states_c3 << candidate_states[i].head(2),candidate_states[i].segment(3,9),candidate_states[i].segment(13,6);
//     UpdateContext_c3(candidate_states_c3);
//     std::vector<double> distances;
//     //std::cerr << "i" << i << std::endl;
//     bool skip_this_i = false;
//     for (int j = 0; j < contact_pairs_c3_.at(0).size(); j++) {
//       // Evaluate the distance for each pair
//       SortedPair<GeometryId> pair {(contact_pairs_c3_.at(0)).at(j)};
//       multibody::GeomGeomCollider collider(plant_c3_, pair);
//
//       auto [phi_i, J_i] = collider.EvalPolytope(*context_c3_, c3_options_.num_friction_directions);
//       //std::cerr << "distance" << phi_i << std::endl;
//       if (phi_i < 0) {
//         skip_this_i = true;
//         break;
//       }
...
//       continue;
//     }
//     candidate_states_temp.push_back(candidate_states[i]);
//   }
//
//   candidate_states = candidate_states_temp;
//   num_total_samples = candidate_states.size();
// }
  ...
  for (int i = 0; i < num_total_samples; i++) {
    ...
    if (sampling_c3_options_.with_z){
      candidate_states_c3 << candidate_states[i];
    }else{
      candidate_states_c3 << candidate_states[i].head(2),candidate_states[i].segment(3,9),candidate_states[i].segment(13,6);
    }
    ...
    resolved_contact_pairs = LCSFactory::PreProcessor(
      plant_c3_, *context_c3_, contact_pairs_c3_,
    ...
    solvers::LCS lcs_object_sample = solvers::LCSFactory::LinearizePlantToLCS(
      plant_c3_, *context_c3_, plant_ad_c3_, *context_ad_c3_, resolved_contact_pairs,
      c3_options_.num_friction_directions,
    ...
    if (sampling_params_.use_different_contacts_to_compute_cost){
      ...
      //TOODO need to check if num_contact will have influence on c3 without z states
      ...
      solvers::LCS lcs_object_sample_for_cost_simulation =
        solvers::LCSFactory::LinearizePlantToLCS(
          plant_c3_, *context_c3_, plant_ad_c3_, *context_ad_c3_,
          resolved_contact_pairs_for_cost_simulation,
          c3_options_.num_friction_directions,
  ...
  #pragma omp parallel for num_threads(num_threads_to_use_)
    for (int i = 0; i < num_total_samples; i++) {
      ...
      if (test_c3_object ->flag_c3 == true) {
        if(!crossed_cost_switching_threshold_){
          if(i == 0){
        ...
        cost_trajectory_pair.first = std::numeric_limits<double>::infinity();
      }
      ...
      if (sampling_c3_options_.with_z) {
        ...
        //   all_sample_dynamically_feasible_plans_.at(i) = cost_trajectory_pair.second;
        //
        //   //std::cerr<< "current trajectory" << cost_trajectory_pair.second[0] << std::endl;
        //
        // }else {
        //   std::vector<Eigen::VectorXd> XX_full(N_+1, VectorXd::Zero(n_x_));
        //   std::vector<Eigen::VectorXd> XX_c3 = cost_trajectory_pair.second;
        //
        //   for (int j = 0; j < N_+1;j++) {
        //     XX_full.at(j).head(2) = XX_c3.at(j).head(2);
        //     XX_full.at(j)[2] = sampling_c3_options_.c3_height;
        //     XX_full.at(j).segment(3,9) = XX_c3.at(j).segment(2,9);
        //     XX_full.at(j)[12] = 0;
        //     XX_full.at(j).segment(13,6) = XX_c3.at(j).segment(11,6);
        //   }
        //
        //   //std::cerr << "other trajectory" << XX_full.at(0) << std::endl;
        //   all_sample_dynamically_feasible_plans_.at(i) = XX_full;
        // }

        //test
          if (test_c3_object -> flag_c3 == false) {
            std::vector<Eigen::VectorXd> XX_full(N_+1, VectorXd::Zero(n_x_));
            all_sample_dynamically_feasible_plans_.at(i) = XX_full;
      ...
      if ((i==CURRENT_REPOSITION_INDEX) && (finished_reposition_flag_==true) && (test_c3_object->flag_c3 == true)) {
  ...
  if (is_doing_c3_ == true) { // Currently doing C3.
    ...
    Eigen::MatrixXd Q_pos_and_rot = Q_[0].block(n_q_franka_,n_q_franka_,7,7);
    Eigen::VectorXd pos_and_rot_error_vec = x_lcs_curr.segment(3, 7) -
      x_lcs_final_des.get_value().segment(3, 7);

    //std::cerr << "Q_pos_and_rot" << Q_pos_and_rot<< std::endl;
    //std::cerr <<"pos_and_rot_error_vec" << pos_and_rot_error_vec << std::endl;
...
void SamplingC3Controller::UpdateContext_c3(Eigen::VectorXd lcs_state) const {
  // Update autodiff.
  VectorXd xu_test(n_q_c3_ + n_v_c3_ + n_u_c3_);

  // u here is set to a vector of 1000s -- TODO why?
  VectorXd test_u = 1000*VectorXd::Ones(n_u_c3_);

  // Update context with respect to positions and velocities associated with
  // the candidate state.
  VectorXd test_q = lcs_state.head(n_q_c3_);
  VectorXd test_v = lcs_state.tail(n_v_c3_);

  xu_test << test_q, test_v, test_u;
  auto xu_ad_test = drake::math::InitializeAutoDiff(xu_test);

  plant_ad_c3_.SetPositionsAndVelocities(
      context_ad_c3_,
      xu_ad_test.head(n_q_c3_ + n_v_c3_));

  multibody::SetInputsIfNew<AutoDiffXd>(
      plant_ad_c3_, xu_ad_test.tail(n_u_c3_), context_ad_c3_);

  plant_c3_.SetPositions(context_c3_, test_q);
  plant_c3_.SetVelocities(context_c3_, test_v);
  multibody::SetInputsIfNew<double>(plant_c3_, test_u, context_c3_);
}
...
void SamplingC3Controller::OutputC3SolutionCurrPlan(
  ...
  c3_solution->x_sol_.resize(n_q_ + n_v_, N_);
  c3_solution->u_sol_.resize(n_u_, N_);
  //std::cerr <<"size of x_sol" << c3_solution->x_sol_.size() << std::endl;
