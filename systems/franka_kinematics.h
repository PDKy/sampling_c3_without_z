...
class FrankaKinematics : public drake::systems::LeafSystem<double> {
 public:
  ...
  const drake::systems::OutputPort<double>& get_output_port_franka_lcs_state() const {
    return this->get_output_port(franka_lcs_state_port_);
  }

  const drake::systems::OutputPort<double>& get_output_port_object_lcs_state() const {
    return this->get_output_port(object_lcs_state_port_);
  }
 ...
 private:
  ...
  //add separate port
  drake::systems::OutputPortIndex franka_lcs_state_port_;
  drake::systems::OutputPortIndex object_lcs_state_port_;
